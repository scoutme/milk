...
0. **`!` direct-execution mode** — pressing `!` as the first character of an empty textarea enters bang mode (Claude Code-style): the `!` is consumed rather than inserted, the prompt symbol switches live to a red `!`, and the rest of the line is typed as plain text. Submitting runs it immediately as a shell command, no confirmation, always taking priority over everything below — including while an agent turn is already in progress: it runs directly via the same embedded-PTY path rather than being funneled into the busy-state "press Enter again to spawn a background agent" flow (that flow is for everything else typed while busy, not bang commands — the agent has no special handling for milk's own `!` convention, so handing it the raw string would be meaningless). Output is appended to the transcript immediately and, if the agent turn was already running, queued to be included in its next turn's context.
0. **Direct-bash shortcut** — when `direct_bash: true` and the input matches the shell-command heuristic, milk prompts `y/N` before running it locally via `sh -c` inside an embedded PTY pane. Skips confirmation if the first token is in `direct_bash_allow`. Fires after slash-command handling but before agent routing. On Windows, where no real PTY is available, milk instead pipes the child's stdout/stderr and streams them into the transcript live (preferring `bash`/`sh` on PATH — Git Bash, MSYS2, WSL interop — falling back to `cmd /C`); the child gets no stdin, so interactive prompts (editors, y/n confirmations) won't work there. WSL itself is unaffected and keeps using the real embedded-PTY path.
