...
4. **Correction to the original plan, then done anyway**: the original plan called `panelTasks`/`tasksOffset` "dead scaffolding" and skipped building a background-jobs panel on that basis. That was wrong — `panelTasks` is a fully working, wired panel (`panel_tasks.go`, toggled via `/panel tasks`, contributing to `mainWidth()`, with its own mouse-wheel scroll region). Once several jobs were running at once in a live session, the transcript line + status-bar count alone weren't enough to see what was actually happening, so a background-agents panel was built after all — `panel_background.go`, an exact structural mirror of `panel_tasks.go`: `backgroundPanelWidth`/`backgroundPanelInner` consts, `buildBackgroundPanelLines([]local.Job, inner) []string`, `renderBackgroundPanel(h int) string`, wired into `layout.go`'s `mainWidth()`/`View()` and `panel_select.go`'s `regionAt`/`panelMaxOffset` (region `regionBackground`, mouse-wheel scroll only — no click-to-select, matching tasks' feature level rather than memory/workflow's). Needed a new `Manager.Jobs() []Job` (value copies, not pointers — `finish()` mutates a `Job`'s fields under `m.mu` from whichever goroutine ran it, so handing out live pointers would let a renderer on the UI goroutine race that write) since nothing previously let anything outside the Manager enumerate jobs for display; `Drain()` intentionally isn't reusable for this since it consumes the pending-delivery queue as a side effect and `Jobs()` must be call-every-render safe.
5. **Also added at this point**: with a fourth toggleable panel, `/panel <name>` alone stopped being convenient — **F1**/**F2**/**F3**/**F4** are new global show/hide shortcuts for memory/tasks/background/workflow respectively (matching the panels' left-to-right join order in `View()`, not build order — background shipped after workflow but sits before it in the shortcut order), intercepted at the very top of `Update`'s `tea.KeyMsg` case (right after the `ptyPane` passthrough guard, before every other mode-specific key handler) so they work in any mode — busy, permission prompt, wizards — since none of those otherwise use function keys and toggling panel width doesn't conflict with anything in progress. Each just calls `handlePanelCmd(name)` directly, so the shortcut and the slash command are two paths to the exact same toggle, not a separate code path to keep in sync. **Found live**: F3/F4 were initially swapped (workflow/background instead of background/workflow) — fixed to match join order, with regression tests (`TestF3TogglesBackgroundPanel`, `TestF4TogglesWorkflowPanel`) pinning the correct mapping.
6. **Added after live-verifying the feature, not in the original plan**: items 1-4 above are all passive — none of them actually generate a response. An escalation agent that had just spawned a wave told the user "I'll follow up automatically when they finish," and nothing made that true. `Manager.SetOnBatchDone(fn func())` (`background.go`) fires exactly once when the last job of the current wave finishes (`ActiveCount()` reaches 0 inside `finish()`, guarded by a `batchSignaled` flag reset on `Drain()` so the next wave can signal again) — deliberately separate from the per-job `SetOnDone`, since firing per-job would produce one redundant follow-up turn per job in a batch rather than one consolidated turn for the whole wave. Wired to a new `backgroundBatchDoneMsg`, handled by `maybeAutoFollowupBackgroundJobs(waitForWholeWave bool)` (`repl.go`): if idle (`!m.busy && m.pendingPerm == nil && m.pendingDirectBash == nil && m.ptyPane == nil`), submits a synthetic prompt (`backgroundFollowupPrompt`, labeled `"[background]"` in the transcript so it reads as distinct from the user's own input) through the exact same `submitInput`/`dispatchAgent` path a real user turn takes; if not idle, sets `pendingBackgroundFollowup` and retries from `handleAgentDone` once the in-flight turn completes — mirroring the pre-existing pattern for queued Telegram remote-oversight input (`m.st.pendingRemoteInputs`) exactly.
7. **Also added after live use, a second real gap**: pressing Enter while the model is busy only ever told the user to wait or interrupt (Ctrl+C) — there was no way to fork off a side-question without either losing it or breaking the current turn. `handleBusyKey`'s plain-Enter case now arms a `busySpawnArmed` flag alongside the existing `busyHint` (same 3s timer, `busyHintClearCmd`) with a "press Enter again to spawn a background agent with this" hint instead of the old "Ctrl+C to interrupt" one (empty input still falls back to the old hint — nothing to spawn from); a second Enter while armed calls `spawnUserBackgroundAgent`, which clears the textarea and calls `Manager.Spawn` directly with `role = "user"` (its own `user:subagent` token bucket — see CLAUDE.md), forking from whichever local-backed agent is available (escalation preferred, primary as fallback; the currently-busy role itself doesn't need to be local-backed — e.g. it could be running on `claude-cli`, which has no Manager at all). Delivery uses a *separate* signal, `backgroundUserJobDoneMsg` (fired per-job from the same `SetOnDone` closure when `Role == "user"`, not `SetOnBatchDone`) and a corresponding `pendingUserBackgroundFollowup` flag, both routed through the same `maybeAutoFollowupBackgroundJobs`, but called with `waitForWholeWave = false`: since the user forked off one specific side-question rather than a coordinated multi-part plan, there's no wave to consolidate with, so it fires as soon as the model is idle regardless of `ActiveCount()` — deliberately more eager than the agent-initiated case.
8. **Found live, a third real bug**: `backgroundFollowupPrompt` (item 6) is a fixed string, submitted verbatim every time a wave finishes. `Run`'s `isRepeatedPrompt` check exists to catch a human repeating themselves out of frustration and self-escalate on their behalf — a milk-generated string recurring across several completed waves looked identical to that from history alone, and was triggering a bogus self-escalation with nothing to do with the user. Fixed by exporting the constant as `local.BackgroundFollowupPrompt` (so `cmd/milk`'s `backgroundFollowupPrompt` is now an alias, not a second textually-identical constant that could drift) and whitelisting it inside `isRepeatedPrompt` itself, rather than at its one current call site — so any future caller inherits the exemption automatically rather than needing to remember to check separately.
9. **Found live, a fourth real bug — panel inconsistency**: the background panel (item 4) mirrored `panel_tasks.go` exactly, including its lack of a scrollbar-thumb indicator showing scroll position. With the memory/tasks/background/workflow panels all competing for screen space, only workflow's (`panel_workflow.go`, pre-existing) had one — and workflow alone also had a left border, so no two panels looked alike. First attempt added a left border to all four (mirroring workflow's `styleWorkflowPanel`) but on review that was the wrong convergence target: a left border is redundant with the scrollbar column already acting as a divider between panels, and it eats inner width for no benefit. Settled on the opposite direction instead — no left border anywhere, just a right-hand scrollbar (dim `│` track / `▌` thumb) on every panel via the memory panel's existing `scrollThumb` helper (new `renderTasksPanelScrollbar`/`renderBackgroundPanelScrollbar`; workflow's border removed, its inner width restored to 32 to match the other three). `panelContentCol` (`panel_select.go`) simplified to a pass-through clamp, since no panel reserves any left-side columns.
10. **Requested immediately after, full parity**: consistent scrolling exposed that click-drag text selection was still memory/workflow-only — tasks and background could scroll but not select or copy their content. Rather than bolt selection onto two more panels by copy-pasting the memory/workflow render logic a third and fourth time, the four panels' render/scrollbar/selection bodies (previously ~90% identical across `panel_memory.go`/`panel_tasks.go`/`panel_background.go`/`panel_workflow.go`) were consolidated into `panel_common.go`: `sidePanelLines(region) (lines []string, inner int)` is the one place mapping a region to its content builder and width; `panelOffsetPtr(region) *int` returns a pointer directly into the model's own offset field (`m.panelOffset`/`m.tasksOffset`/`m.backgroundOffset`/`m.workflowPanelOffset`), letting `handleMouse`'s wheel-scroll cases collapse from a 4-armed switch to `if p := m.panelOffsetPtr(region); p != nil { ... } else { viewport scroll }`; `renderSidePanel`/`renderSidePanelScrollbar` implement the shared body/scrollbar once, with every `render<Name>Panel*` now a one-line wrapper. `handleMouse`'s `MouseButtonLeft` gate generalized from listing two regions to `region != regionNone`, so tasks/background now reach `handlePanelMouse` exactly like memory/workflow. The only surviving panel-specific logic is content generation (`build<Name>PanelLines`, genuinely different per panel) and the memory panel's double-click-for-detail lookup, still gated to `region == regionMemory` inside the shared `handlePanelMouse` — a deliberate exception, not a leftover asymmetry.
11. **Requested immediately after that, divider parity**: tasks/background drew a `───` rule under the title (tasks drew a second one between SESSION/GLOBAL); memory/workflow just used a blank line in the same spot. Removed the rule from tasks/background rather than adding one to memory/workflow — one line of separation conveys the same grouping either way, and dropping two now-unused `hr()` closures (and the `strings` import that only existed for one of them, in `panel_tasks.go`) is strictly less code than adding two more.
12. **Found live, a fifth real bug — job timeout too tight**: a live session's two background jobs (multi-file terrain-analysis on a heavier reasoning model) were still issuing new tool-loop requests 8+ minutes into their run when the original 10-minute `defaultJobTimeout` killed them mid-progress, not mid-hang — confirmed from `local_debug.log`/`~/.milk/otel/milk.log` timestamps, not guessed. The fix isn't "make the timeout bigger and hope" — it's recognizing the timeout was never the primary defense against a *stuck* job in the first place: loop detection (streak tracker, streaming n-gram monitor, duplicate-tool-call detection) already runs unmodified on a background job, since `cloneForBackground` leaves `workflowRole` false specifically so none of the workflow-role skip conditions apply to it. An actually-repeating job gets caught by that, well before any wall-clock timeout. So the timeout's real job is narrower — catch a job that's genuinely still working but will never finish — and can afford to be generous. Raised `defaultJobTimeout` 10→20 minutes, and made it configurable (`background_agent_timeout_minutes` in `Config`, `EffectiveBackgroundAgentTimeout()`, mirroring `max_background_agents`'s exact pattern) rather than only bumping the constant, since the right value depends on model speed and task size — wired via `agents.backgroundMgr.SetJobTimeout(cfg.EffectiveBackgroundAgentTimeout())` right after `Manager` construction in `repl.go`.
