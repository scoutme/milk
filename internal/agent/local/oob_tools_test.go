...
func TestExecuteToolCalls_ConcurrentDispatch(t *testing.T) {
	...
	start := time.Now()
	msgs, esc := a.executeToolCalls(ctx, nil, tcs, "", "", io.Discard, nil, nil, "", nil)
...
func TestExecuteToolCalls_ContextCancellation(t *testing.T) {
	...
	go func() {
		defer close(done)
		a.executeToolCalls(ctx, nil, tcs, "", "", io.Discard, nil, nil, "", nil) //nolint:errcheck
	}()
...
func TestExecuteToolCalls_ResultOrder(t *testing.T) {
	...
	ctx := context.Background()
	msgs, _ := a.executeToolCalls(ctx, nil, tcs, "", "", io.Discard, nil, nil, "", nil)
