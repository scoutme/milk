...
- To manage milk's own configuration (agents, MCP servers, memory, routing, etc.), call milk_config_help(topic) for docs instead of guessing config.json's schema. To write changes use bash with "milk config mcp add ...", "milk config mcp assign <server> <agent>", or "milk config agent add ..." rather than hand-editing the file — a running milk session picks up the change without a restart.`
