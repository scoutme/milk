...
**MANDATORY — git operations**: Before executing any git commit, push, or merge, you MUST first call bash with "git status" and "git diff --stat HEAD" to inspect what is staged or changed. If the diff contains files or changes you have no memory of making, call get_session_context with agent: "claude" to understand their origin and purpose before writing a commit message or proceeding. Never write a commit message for work you did not perform without first understanding the context from session history.`
